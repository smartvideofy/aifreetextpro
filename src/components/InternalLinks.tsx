import { Link } from "react-router-dom";
import { ArrowRight, BookOpen, Shield, Target, Users, Sparkles } from "lucide-react";

interface InternalLinksProps {
  currentPage?: string;
  showTitle?: boolean;
  variant?: "compact" | "full" | "inline";
}

// Key internal pages to link to for SEO
const keyPages = [
  {
    title: "AI Detector",
    href: "/ai-checker",
    description: "Check if text is AI-generated with 98% accuracy",
    icon: Shield,
    category: "tool"
  },
  {
    title: "Bypass Turnitin",
    href: "/bypass-turnitin-ai-detection",
    description: "Make AI content undetectable by Turnitin",
    icon: Target,
    category: "bypass"
  },
  {
    title: "Bypass GPTZero",
    href: "/bypass-gptzero-detection",
    description: "Pass GPTZero AI detection",
    icon: Target,
    category: "bypass"
  },
  {
    title: "Bypass Originality.AI",
    href: "/bypass-originality-ai",
    description: "Humanize content for Originality.AI",
    icon: Target,
    category: "bypass"
  },
  {
    title: "For Students",
    href: "/ai-humanizer-for-students",
    description: "AI humanizer designed for academic use",
    icon: Users,
    category: "usecase"
  },
  {
    title: "For Writers",
    href: "/ai-humanizer-for-writers",
    description: "Professional AI writing tools for content creators",
    icon: Users,
    category: "usecase"
  },
  {
    title: "Academic Writing",
    href: "/academic-ai-writing-tool",
    description: "AI tools for thesis and essay writing",
    icon: BookOpen,
    category: "usecase"
  },
  {
    title: "vs WriteHuman",
    href: "/vs-writehuman",
    description: "Compare AI Free Text Pro to WriteHuman",
    icon: Sparkles,
    category: "compare"
  },
  {
    title: "vs Undetectable AI",
    href: "/vs-undetectable-ai",
    description: "AI Free Text Pro vs Undetectable.AI comparison",
    icon: Sparkles,
    category: "compare"
  },
  {
    title: "vs HumanizeAI Pro",
    href: "/vs-humanizeai-pro",
    description: "AI Free Text Pro vs HumanizeAI.pro comparison",
    icon: Sparkles,
    category: "compare"
  },
  {
    title: "Compare All Humanizers",
    href: "/compare-ai-humanizers",
    description: "Side-by-side AI humanizer comparison",
    icon: Sparkles,
    category: "compare"
  },
  {
    title: "Free AI Detector",
    href: "/ai-checker",
    description: "Free online AI content detection tool",
    icon: Shield,
    category: "tool"
  },
  {
    title: "Pricing",
    href: "/pricing",
    description: "View plans and pricing",
    icon: Shield,
    category: "main"
  },
  {
    title: "Blog",
    href: "/blog",
    description: "AI writing tips and guides",
    icon: BookOpen,
    category: "main"
  }
];

const blogPosts = [
  // High-priority 2026 hub posts
  { title: "Best AI Detector 2026", href: "/blog/best-ai-detector-2026" },
  { title: "Best Free AI Humanizer 2026", href: "/blog/best-free-ai-humanizer-2026" },
  { title: "Top 10 AI Writing Tools 2026", href: "/blog/top-10-ai-writing-tools-2026" },
  { title: "Turnitin vs GPTZero vs Originality", href: "/blog/turnitin-vs-gptzero-vs-originality-ai" },
  { title: "AI Detectors: Claude, Gemini, GPT-5", href: "/blog/ai-detectors-claude-gemini-gpt5-2026" },
  { title: "Can Detectors Catch GPT-5?", href: "/blog/can-ai-detectors-detect-gpt5" },
  // Core education
  { title: "How AI Detectors Work", href: "/blog/how-ai-detectors-work" },
  { title: "How Detectors Score Text", href: "/blog/how-ai-detectors-score-text" },
  { title: "AI Detection Patterns", href: "/blog/ai-detection-patterns-explained" },
  { title: "Signal vs Noise in Human Text", href: "/blog/signal-vs-noise-human-text" },
  { title: "AI Detection False Positives", href: "/blog/ai-detection-false-positives" },
  // Humanizer guides
  { title: "Humanize AI Text Guide", href: "/blog/humanize-ai-text-without-losing-voice" },
  { title: "Humanize ChatGPT Text", href: "/blog/humanize-chatgpt-text" },
  { title: "Humanize Claude Output", href: "/blog/humanize-claude-output" },
  { title: "Humanize Gemini Output", href: "/blog/humanize-gemini-output" },
  { title: "Paraphrasing vs Humanizing", href: "/blog/paraphrasing-vs-humanizing" },
  { title: "AI Humanization Pitfalls", href: "/blog/ai-humanization-pitfalls" },
  { title: "Top 10 AI Humanizers", href: "/blog/top-10-ai-humanizer-tools-2025" },
  { title: "Best Free AI Humanizers", href: "/blog/best-free-ai-humanizer-tools-2025" },
  // Bypass guides
  { title: "Pass All AI Detectors", href: "/blog/pass-all-ai-detectors-guide" },
  { title: "Bypass AI Detection Guide", href: "/blog/bypass-ai-detection-guide" },
  { title: "Make ChatGPT Undetectable", href: "/blog/make-chatgpt-undetectable-2025" },
  { title: "Bypass Copyleaks", href: "/blog/bypass-copyleaks-ai-detection" },
  { title: "Bypass Winston AI", href: "/blog/bypass-winston-ai-detection" },
  { title: "Bypass ZeroGPT", href: "/blog/bypass-zerogpt-ai-detection" },
  // Detector deep-dives
  { title: "Turnitin AI Accuracy", href: "/blog/turnitin-ai-detection-accuracy" },
  { title: "Turnitin Similarity vs AI Score", href: "/blog/turnitin-similarity-vs-ai-score" },
  { title: "Appeal Turnitin Results", href: "/blog/turnitin-appeal-ai-detection" },
  { title: "GPTZero Accuracy Review", href: "/blog/gptzero-accuracy-review" },
  { title: "GPTZero vs Turnitin", href: "/blog/gptzero-vs-turnitin-comparison" },
  { title: "Originality.AI Review", href: "/blog/originality-ai-review-how-to-beat" },
  { title: "Originality.AI Accuracy", href: "/blog/originality-ai-accuracy-false-positives" },
  { title: "Copyleaks vs Turnitin", href: "/blog/copyleaks-vs-turnitin" },
  // Comparisons
  { title: "ZeroGPT Comparison", href: "/blog/zerogpt-vs-ai-free-text-pro-2025" },
  { title: "GPTinf Comparison", href: "/blog/gptinf-vs-ai-free-text-pro" },
  { title: "Quillbot Comparison", href: "/blog/quillbot-vs-ai-free-text-pro" },
  { title: "Wordtune Comparison", href: "/blog/wordtune-vs-ai-free-text-pro" },
  { title: "Undetectable AI Deep Dive", href: "/blog/undetectable-ai-vs-ai-free-text-pro" },
  { title: "Undetectable AI Alternatives", href: "/blog/undetectable-ai-alternatives" },
  { title: "Stealthwriter Alternative", href: "/blog/stealthwriter-alternative" },
  // Use cases / verticals
  { title: "AI Humanizer for Essays", href: "/blog/ai-humanizer-for-essays" },
  { title: "AI Humanizer for Blog Posts", href: "/blog/ai-humanizer-for-blog-posts" },
  { title: "AI for Dissertation & Thesis", href: "/blog/ai-dissertation-thesis" },
  { title: "AI for Research Papers", href: "/blog/ai-for-research-papers" },
  { title: "AI for College Admissions", href: "/blog/ai-college-admissions-essays-personal-statements" },
  { title: "AI Writing for Non-Native Speakers", href: "/blog/ai-writing-non-native-english-speakers" },
  { title: "AI Writing for Students", href: "/blog/ai-writing-students-avoid-plagiarism" },
  { title: "AI for High School Students", href: "/blog/ai-writing-high-school-students" },
  { title: "Best AI Tools for Students", href: "/blog/best-ai-tools-for-students" },
  { title: "ChatGPT Prompts for Essays", href: "/blog/best-chatgpt-prompts-for-essays" },
  { title: "AI Cover Letter Generator", href: "/blog/ai-cover-letter-generator" },
  { title: "AI Humanizer for Resumes", href: "/blog/ai-humanizer-resumes-cover-letters" },
  { title: "AI Content for SEO", href: "/blog/ai-content-seo-undetectable" },
  { title: "AI-Powered SEO Content 2026", href: "/blog/ai-powered-seo-content-2026" },
  { title: "AI Content & Google AI Overviews", href: "/blog/ai-content-google-ai-overviews" },
  // Verification & policy
  { title: "Can Teachers Detect ChatGPT?", href: "/blog/can-teachers-detect-chatgpt" },
  { title: "Can Turnitin Detect DeepSeek?", href: "/blog/can-turnitin-detect-deepseek" },
  { title: "Can Turnitin Detect Perplexity?", href: "/blog/can-turnitin-detect-perplexity" },
  { title: "Does Canvas Detect ChatGPT?", href: "/blog/does-canvas-detect-chatgpt" },
  { title: "Does Turnitin Detect Gemini?", href: "/blog/does-turnitin-detect-gemini" },
  { title: "Check If Written by AI", href: "/blog/how-to-check-if-written-by-ai" },
  { title: "Check My Essay for AI", href: "/blog/check-my-essay-for-ai" },
  { title: "AI Disclosure Policies 2026", href: "/blog/ai-disclosure-policies-2026" },
  { title: "Is AI for School Illegal?", href: "/blog/is-it-illegal-to-use-ai-for-school" },
  { title: "How to Cite AI Content", href: "/blog/how-to-cite-ai-generated-content" },
  { title: "AI Detector for Employers", href: "/blog/ai-detector-employers" },
  { title: "ChatGPT for College Essays", href: "/blog/chatgpt-college-essays" },
  { title: "AI Text Converter", href: "/blog/ai-text-converter" },
  { title: "Claude Academic Writing", href: "/blog/claude-academic-writing" },
];

export const InternalLinks = ({ currentPage, showTitle = true, variant = "compact" }: InternalLinksProps) => {
  // Filter out current page from links
  const filteredPages = keyPages.filter(page => page.href !== currentPage);
  const filteredBlogPosts = blogPosts.filter(post => post.href !== currentPage);
  
  if (variant === "inline") {
    return (
      <div className="flex flex-wrap gap-2 text-sm">
        {filteredPages.slice(0, 6).map((page) => (
          <Link 
            key={page.href} 
            to={page.href}
            className="text-primary hover:text-primary/80 hover:underline"
          >
            {page.title}
          </Link>
        ))}
      </div>
    );
  }

  if (variant === "compact") {
    return (
      <div className="my-8 p-6 bg-muted/50 rounded-lg border border-border">
        {showTitle && (
          <h3 className="font-semibold mb-4 flex items-center gap-2">
            <BookOpen className="w-4 h-4 text-primary" />
            Related Resources
          </h3>
        )}
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {filteredPages.slice(0, 6).map((page) => (
            <Link 
              key={page.href} 
              to={page.href}
              className="text-sm text-muted-foreground hover:text-primary transition-colors flex items-center gap-1 group"
            >
              <ArrowRight className="w-3 h-3 group-hover:translate-x-0.5 transition-transform" />
              {page.title}
            </Link>
          ))}
        </div>
        
        <div className="mt-4 pt-4 border-t border-border">
          <p className="text-sm font-medium mb-2">Popular Blog Posts:</p>
          <div className="grid grid-cols-2 gap-2">
            {filteredBlogPosts.slice(0, 6).map((post) => (
              <Link 
                key={post.href} 
                to={post.href}
                className="text-sm text-muted-foreground hover:text-primary transition-colors"
              >
                → {post.title}
              </Link>
            ))}
          </div>
        </div>
      </div>
    );
  }

  // Full variant
  return (
    <section className="py-12 bg-muted/30">
      <div className="container mx-auto px-4">
        {showTitle && (
          <h2 className="text-2xl font-bold mb-8 text-center">Explore More AI Tools & Resources</h2>
        )}
        
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          {/* Bypass Tools */}
          <div>
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <Target className="w-4 h-4 text-primary" />
              Bypass Guides
            </h3>
            <ul className="space-y-2">
              {filteredPages.filter(p => p.category === "bypass").map((page) => (
                <li key={page.href}>
                  <Link to={page.href} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                    {page.title}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Use Cases */}
          <div>
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <Users className="w-4 h-4 text-primary" />
              Use Cases
            </h3>
            <ul className="space-y-2">
              {filteredPages.filter(p => p.category === "usecase").map((page) => (
                <li key={page.href}>
                  <Link to={page.href} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                    {page.title}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Compare */}
          <div>
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-primary" />
              Compare Tools
            </h3>
            <ul className="space-y-2">
              {filteredPages.filter(p => p.category === "compare").map((page) => (
                <li key={page.href}>
                  <Link to={page.href} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                    {page.title}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Blog Posts */}
          <div>
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <BookOpen className="w-4 h-4 text-primary" />
              Popular Guides
            </h3>
            <ul className="space-y-2">
              {filteredBlogPosts.slice(0, 5).map((post) => (
                <li key={post.href}>
                  <Link to={post.href} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                    {post.title}
                  </Link>
                </li>
              ))}
            </ul>
          </div>
        </div>
      </div>
    </section>
  );
};

export default InternalLinks;
